...
export const createVisitorRequest = async (req, res, next) => {
  try {
    ...
    if (!day) throw createError(400, 'The house have no schedule this day');

    let start = new Date().setHours(day.starttime.getHours(), day.starttime.getMinutes(), 0, 0);
    ...
    console.log('here', day.starttime, date, day.endtime);
    ...
    for (let index = 0; index < schedules.length; index++) {
      if (schedules[index].visitor.toString() === visitor._id.toString()) continue;
...
export const getOwnerRequests = async (req, res, next) => {
  try {
    ...
    if (req.query.end) searchParam.date = { ...searchParam.date, $lte: new Date(req.query.end) };
    ...
    const requests = await Requests.aggregate([
      ...
      },
      {
        $addFields: {
          fullname: {
            $concat: [{ $first: '$visitor.firstname' }, ' ', { $first: '$visitor.lastname' }],
          },
        },
      },
      { $match: searchName },
