...
export const addTenant = async (req, res, next) => {
  ...
  try {
    ...
    if (user && user.role != 'user') throw createError(400, 'This email has been taken!!');
    if (!user) {
      user = new User({
        role: 'tenant',
        email,
    ...
    const tenant = await Tenant.create([{ user, reference, national_id, mother_name }], {
      session,
    });
    const house = await House.findById(houseId);

    if (!house) throw createError(400, 'House not found!');
    if (house.tenant)
      throw createError(400, 'You have to remove that tenant before adding a new one');

    house.occupancy_history = [
      ...house.occupancy_history,
      {
        tenant: user._id,
    ...
    if (req.files) {
      if (req.files.nationalid)
        await removeImage(
          req.files['nationalid'][0].destination + '/' + req.files['nationalid'][0].filename
        );
      if (req.files.contract)
        await removeImage(
          req.files['contract'][0].destination + '/' + req.files['contract'][0].filename
        );
    }
    next(error);
  } finally {
...
export const editTenant = async (req, res, next) => {
  ...
  try {
    ...
    if (reference) reference = JSON.parse(reference);
    const user = await User.findOne({ _id: req.user, isActive: true }).select(
    ...
    if (removedPath !== '') await removeImage(removedPath);
...
export const getTenant = async (req, res, next) => {
  try {
    ...
    else user = await User.findOne({ _id: req.params.id }).select('-password');
    ...
    if (req.role === 'owner')
      house = await House.aggregate([
        ...
        { $project: { occupancy_history: 1, deadline: 1, contract: 1 } },
        { $unwind: '$occupancy_history' },
        { $match: { 'occupancy_history.tenant': new mongoose.Types.ObjectId(user._id) } },
        {
          $addFields: {
    ...
    else house = [await House.findOne({ tenant: user._id }).select('deadline contract')];

    if (!house) throw createError(400, 'Not allowed to see this tenant');
    const tenant = await Tenant.findOne({ user: user._id });

    if (!tenant) throw createError(400, 'Not a tenant');
...
export const deleteTenant = async (req, res, next) => {
  try {
    ...
    if (!tenant) throw createError(400, "This house doesn't have a tenant!!");

    house.occupancy_history = house.occupancy_history.map(history => {
      if (history.tenant.toString() === tenant.toString()) history.upto = Date.now();
      return history;
    });
...
export const getOwner = async (req, res, next) => {
  try {
    ...
    const house = await House.findOne({ tenant: tenant._id })
      .select('owner')
      .populate({
        path: 'owner',
        foreignField: 'user',
    ...
    if (!house || !house.owner) throw createError(400, 'Owner not found!!');
