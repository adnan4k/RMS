...
function ScheduleVisit({ calendar, id }) {
  ...
  const onSubmit = () => {
    ...
    else toast.error('Please right a short message');
  };

  useEffect(() => {
    if (data && data.date) {
      setChoosenDate(dayjs(data.date.date));
      if (message.current) message.current.value = data.date.message;
    }
  }, [data, status]);

  console.log(serror, 'serror');
  return (
    <div className="flex justify-between min-h-64">
      <div className="flex flex-col h-full w-full mr-4 justify-around">
          ...
          <DateTimePicker
            ...
            shouldDisableTime={(time, type) => {
              if (type === 'hours') {
                ...
                if (now.isAfter(endtime) || now.isBefore(starttime)) return true;
                ...
                if (s) {
                  if (
                    s.filter(
                      ({ date }) =>
                        time.isBefore(dayjs(date).subtract(1, 'hour')) ||
                        time.isBefore(dayjs(date).add(1, 'hour'))
                    ).length > 0
                  )
                    return true;
                }
        ...
        <textarea
          rows={2}
          ref={message}
          className="my-2 border p-2 rounded-lg"
          placeholder="Write a short message to the owner"
        ></textarea>
        <button
          className={`bg-blue-600 ${serror && 'cursor-not-allowed'}`}
          onClick={onSubmit}
          disabled={serror !== null}
        >
          Schedule
        </button>
      </div>
      <div className="ml-2 w-full text-center">
        <div className="w-full max-w-sm p-2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-auto">
          <h5 className="mb-3 text-base font-semibold text-gray-900 md:text-xl dark:text-white">
            {data?.requests.length > 0 ? 'Booked Dates' : 'No Booked dates yet'}
          </h5>
          <p className="text-sm font-normal text-gray-500 dark:text-gray-400">
            {data?.requests.length > 0
              ? 'The below dates are already booked by other users. So they are disabled on the calendar'
              : 'Be the first one to book a visit to this house'}
          </p>
          <ul className="my-4 space-y-3">
            {data?.requests.map(dates => (
              <li key={dates._id}>
                <div
                  onClick={() => onClick(dates._id)}
                  className="flex cursor-pointer items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white"
                >
                  <span className="flex-1 ms-3 whitespace-nowrap">
                    {dayjs(dates._id).format('MMM DD YYYY')}
                  </span>
                  <span className="inline-flex items-center justify-center px-1 py-0.5 ms-3 text-xs font-medium text-gray-500 bg-gray-200 rounded dark:bg-gray-700 dark:text-gray-400">
                    {dates.count} requests
                  </span>
                </div>
                <div
                  id="dropdown"
                  className={
                    'mt-1 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 ' +
                    (selected === dates._id ? '' : 'hidden')
                  }
                >
                  {dates.requests.map(({ date }) => (
                    <ul className="py-2" aria-labelledby="dropdownButton">
                      <li>
                        <div className="block px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
