...
export const ProtectedRoutes = ({role}) => {
    ...
    if (status === 'pending' || !data) 
        return (
        <svg aria-hidden="true" role="status" className="self-center mt-5 inline w-24 h-24 me-3 text-gray-200 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    if (status === 'success' && role === 'tenant') {
        return <Outlet context={data}/>
    }
    if (status === 'success' && data.role === 'tenant')
