      ...
      <div className="flex justify-between h-screen">
        <div className="flex flex-col mr-3 min-w-64 overflow-y-scroll">
          <div className="flex items-center peer border border-gray-300 rounded-lg bg-gray-50 dark:bg-gray-700 mb-3 dark:border-gray-600">
            <svg
              className="w-8 h-8 p-1 pointer-events-none text-gray-500 dark:text-gray-400 rounded-l-lg m-1"
              aria-hidden="true"
            ...
            <input
              ...
              value={search}
              className="block w-full p-2 text-sm text-gray-900 border border-gray-300 bg-gray-50 outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder="Search house, ..."
            />
            <button
              onClick={searchHouse}
              className="p-1 bg-blue-500 mx-1 text-sm rounded-lg border"
            >
              Search
            </button>
          </div>
          ...
          <Link
            ...
            className="min-h-10 w-full p-2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 text-lg dark:text-white"
            to="/houses"
          >
            Clear all filters
          </Link>
            ...
            {DEFAULT_TYPES.map(({ name, img }) => (
              <>
                <label
                  htmlFor={name}
                  className={
                    'border flex flex-col p-1.5 pb-0 mx-2 cursor-pointer border-gray-200 dark:border-gray-700 h-full w-20 min-w-20 rounded ' +
                    (searchParams.has('types', name.toLowerCase())
                      ? 'dark:bg-gray-500'
                      : 'dark:bg-gray-800')
                  }
                >
                  <img
                    src={'/images/' + img}
                    className="w-full h-1 flex-1 rounded-full"
                    alt="image"
                  />
          ...
          <div className="grid gap-10 grid-cols-3 w-full px-2">
            {datas.data.length === 0 || status === 'error' ? (
              <>
                ...
                <div className="w-64 h-64 m-auto">
                  ...
                  <p className="text-center">
                    No houses found!{' '}
                    <Link onClick={() => setSignal(!signal)} to="">
                      {' '}
              ...
              datas.data.map((data, index) => {
                ...
                return <HouseAdv key={index} image={image} {...data} />;
              })
            )}
          </div>
          {datas.data.length > 0 && (
            <div className="flex flex-col items-center self-end p-2">
              <span className="text-sm text-gray-700 dark:text-gray-400">
                ...
                of{' '}
                <span className="font-semibold text-gray-900 dark:text-white">{datas?.total}</span>{' '}
                Entries
              </span>
